  ...
  sink() {
    ...
    while (this.hasLeftChild(current)) {
        ...
        if (this.hasRightChild(current) && 
            !this.pairIsInCorrectOrder(this.leftChild(current), this.rightChild(current))) {
            comparisonIndex = this.getRightChildIndex(current);
        }

        // Stop sinking if the current node is already in the correct order
        if (this.pairIsInCorrectOrder(this.container[current], this.container[comparisonIndex])) {
            break;
        }
...
var lastStoneWeight = function (stones) {
  const q = new Heap((f, l) => (f > l ? -1 : 1),stones);
